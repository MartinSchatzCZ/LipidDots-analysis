function [ outputDots, outputCells] = fcn_POMBE2(B_Files,N_B,G_Files,~,Z,dir)
% Function to process POMBE
outputDots = [];
outputCells = [];
...
for i=1:N_B
    ...
    %% SET detection threshold [TH_cell] from the middle slice
        ...
        TH_cell=floor(((min(I2(:))-max(I2(:)))*0.25));
        oBW2 = zeros(size(I2));
        oBW2(I2 < TH_cell)=1;
...
[Cellso,~]=fcn_intCells(Cells,1); % Interpolation of the space between detected slices
