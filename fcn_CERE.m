function [ outputDots, outputCells] = fcn_CERE(B_Files,N_B,G_Files,~,Z,dir)
% Function to process CERE
outputDots = [];
outputCells = [];
...
for i=1:N_B
    ...
    TH_cell = floor(((min(I2(:))-max(I2(:)))*0.42));
...
%% Droplets detection 
    ...
    %% Segmentation
        ...
        BWdots = BWdots.*Cells(:,:,z);
        Dots(:,:,z) = bwareaopen(BWdots,4);      % Remove dots smaller than 4 px
        [BB,~] = bwboundaries(Dots(:,:,z),'noholes');
